...
public class RedBlackTree {
    ...
    public String treeDelete(int x,Stack<RedBlackTree> treeStack) {
    	String messaggio = "";
         //cerco se nell'albero è presente un nodo con chiave key
    	Nodo u = this.find(x);
   ...
   public void treeInsert(int x,Text txtComments,Stack<RedBlackTree> treeStack) {
        txtComments.setText("");
   ...
   public void clonaAlbero(Nodo root,Nodo cloneRoot) {
	   ...
	   if(root.left!=null){
		   cloneRoot.left=new Nodo(root.left.key);
		   clonaAlbero(root.left,cloneRoot.left);   
	   }
	   if(root.left!=null){
		   cloneRoot.right=new Nodo(root.right.key);
		   clonaAlbero(root.right,cloneRoot.right);   
	   }
